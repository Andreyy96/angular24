...
import {IUser} from "../interfaces/user.inteerface";
...
export class AuthService {
  ...
  register(user: IAuth): Observable<IUser>{
    console.log(user)
    return this.httpClient.post<IUser>(urls.auth.register, user)
  }
  ...
  me(): Observable<IUser> {
    return this.httpClient.get<IUser>(urls.auth.me).pipe(tap(user => this.setAuthUser(user)))
  }
